...
# Cria a tabela de respostas no banco de dados
def criar_tabela():
...
if 'perguntas' not in st.session_state:
    ...
    st.session_state.respostas = [""] * len(st.session_state.perguntas)

if st.button("Começar teste") and nome and idade and funcao:
    st.session_state.mostrar_perguntas = True

if 'mostrar_perguntas' in st.session_state and st.session_state.mostrar_perguntas:
    for i, (pergunta, alternativas) in enumerate(st.session_state.perguntas.items()):
        st.write(pergunta)
        resposta = st.radio(label="label", options=alternativas, key=f"pergunta_{i}", label_visibility="hidden")
    ...
    if st.button("Finalizar Teste"):
        ...
        inserir_resposta(nome, idade, funcao, acertos)

        # Agradecimento ao usuário
        st.write("Obrigado por participar do teste!")
        st.write(f"Você acertou {acertos} de {len(st.session_state.respostas)} perguntas.")
        if acertos < len(st.session_state.respostas):
            st.write(f"você errou as questões: {', '.join([str(i+1) for i, resposta in enumerate(st.session_state.respostas) if resposta != respostas_corretas[i]])}")
        else:
            st.write("Parabéns! Você acertou todas as perguntas!")